...
public class Helpers
{
    ...
    public class BleAttributeDisplay
    {
        ...
        public bool CanRead
        {
            get { return this.characteristic != null ? this.characteristic.CharacteristicProperties.HasFlag(GattCharacteristicProperties.Read) : false; }
        }
        ...
        {
            get { return this.characteristic != null ? this.characteristic.CharacteristicProperties.HasFlag(GattCharacteristicProperties.Notify) : false; }
        }
        ...
        public static string GetIdByNameOrNumber(object collection, string name)
        {
            ...
            if (name[0] == '#')
            {
                ...
                if (int.TryParse(name.Substring(1), out devNumber))
                {
                    ...
                    {
                        if (0 <= devNumber && devNumber < (collection as List<BleAttributeDisplay>).Count)
                        {
                            result = (collection as List<BleAttributeDisplay>)[devNumber].Name;
                        }
                    }
                }
                else if (!Console.IsOutputRedirected)
            ...
            else
            {
                ...
                {
                    var foundDispAttrs = (collection as List<BleAttributeDisplay>).Where(d => d.Name.ToLower().StartsWith(name.ToLower())).ToList();
